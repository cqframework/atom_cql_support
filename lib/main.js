...
class CQLLanguageClient extends AutoLanguageClient {
  ...
  startServerProcess() {
    ...
    const serverHome = path.join(__dirname, '..', 'language-server2')
    const jarpath = path.join(serverHome, 'cql-language-server.jar');
    const command = this.getJavaCommand();
    let javaVersion;

    this.installServer(serverHome);
...
installServer (serverHome) {
  const serverDownloadUrl = path.join('https://oss.sonatype.org/content/repositories/snapshots/info/cqframework/cql.ls', packagejson.languageServerVersion
);
  ...
  const localFileName = path.join(serverHome, 'cql-language-server.jar')
