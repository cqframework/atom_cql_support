...
// Installs a jar using maven coordinates
 ...
 // }
  ...
  async executeModelCQL(editor) {
    ...
    const resultsPane = atom.workspace.getActivePane().splitRight({ items: [ resultsTextEditor ], copyActiveItem: false });
...
//this should work, but the cql-cli needs to look for data below the library name
        ...
        const result = this.executeCQL(editor, command, jarPath, libraryDirectory, libraryName, modelPathSpecifier, terminologyPath, contextSpecifier, resultsTextEditor);
  ...
  async executeCQL(editor, command, jarPath, libraryDirectory, libraryName, modelPathSpecifier, terminologyPath, contextSpecifier, resultsTextEditor){
    var args = [
      //debug
      //'-Xdebug',
...
//On Windows the args could error due to spaces in the path
    cp.exec(`"${command}" ${args.join(' ')}`, (error, stdout, stderr) => {
      ...
      resultsTextEditor.insertText(`${contextSpecifier}\r\n${stdout.toString()}`);
