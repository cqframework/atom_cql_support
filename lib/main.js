...
//todo: get this working (currently errors with: Index 0 out of bounds for length 0)
    ...
    atom.workspace.open(`${path.join(resultPath, libraryPathName + '.txt')}`,  {
      cursorBlink: false,
      visualBell: true,
      convertEol: true,
      ...
      textEditor.insertText('Running tests.\r\n');
      textEditor.insertText(`${modelMessage}\r\n`);
      textEditor.insertText(`${terminologyMessage}\r\n`);
      return textEditor;
    }).then(textEditor => {
      if (modelRootPath && modelRootPath != '') {
        ...
        if (dirs && dirs.length > 0) {
          dirs.forEach(dirent => {
            ...
            const modelPathSpecifier = modelTypeSpecifier + modelRootPath + "/" + dirent.name;
            const contextSpecifier = contextTypeSpecifier + context;
            this.executeCQL(textEditor, editor, command, jarPath, libraryDirectory, libraryName, fhirVersion, modelPathSpecifier, terminologyPath, contextSpecifier, context, measurementPeriod, verbose);
          });
        }
        else {
          this.executeCQL(textEditor, editor, command, jarPath, libraryDirectory, libraryName, fhirVersion, null, terminologyPath, null, null, measurementPeriod, verbose);
        }
      }
      else {
        this.executeCQL(textEditor, editor, command, jarPath, libraryDirectory, libraryName, fhirVersion, null, terminologyPath, null, null, measurementPeriod, verbose);
...
//this results display should be in the caller, but I keep getting a null from the promise that's returned
        textEditor.insertText(`Test (${testName})\r\n${new Date().toString()}\r\n${error}\r\n`);
        return;
      }
...
//this results display should be in the caller, but I keep getting a null from the promise that's returned
      ...
      textEditor.insertText(`${stdout.toString()}elapsed: ${((endExecution - startExecution) / 1000).toString()} seconds\r\n\r\n`);
    });
  }
