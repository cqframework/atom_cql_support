...
//todo: make this a setting
    ...
    if ( (!terminologyPath || /^\s*$/.test(terminologyPath))) {
      terminologyPath = path.join(projectPath, 'input/vocabulary/valueset');
    }
...
//todo: get this working (currently errors with: Index 0 out of bounds for length 0)
    ...
    atom.workspace.open(`${path.join(projectPath, 'input/tests', 'results', libraryPathName + '.txt')}`,  {
      cursorBlink: false,
      visualBell: true,
      convertEol: true,
      ...
      if (modelRootPath && modelRootPath != '') {
        ...
        if (dirs && dirs.length > 0) {
          ...
          this.executeCQL(editor, command, jarPath, libraryDirectory, libraryName, null, terminologyPath, null, null, measurementPeriod, verbose);
        }
      }
      else {
        this.executeCQL(editor, command, jarPath, libraryDirectory, libraryName, null, terminologyPath, null, null, measurementPeriod, verbose);
