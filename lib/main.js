  ...
  startServerProcess() {
    ...
    return this.checkJavaVersion(command)
      .then(foundJavaVersion => {
        ...
        return this.installServerIfRequired(serverHome, coords, "language server");
      })
      .then(() => {
        var args = [
            '-cp',
...
installServerIfRequired (serverHome, coords, displayName) {
  return this.isServerInstalled(serverHome, coords)
    .then(doesExist => { if (!doesExist) 
        return this.installJar(serverHome, coords, displayName);
    });
}

isServerInstalled (serverHome, coords) {
