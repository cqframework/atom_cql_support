...
// Installs a jar using maven coordinates
  installJar(home, coords, displayName) {
    ...
    return this.fileExists(localPath)
 ...
 // }
  ...
  addCQLExecuteMenu(connection) {
    const executeCQLCommand= atom.commands.add(
      ...
      async (e) => {
        this.executeCQLFile(e.currentTarget)
      }
    );
...
//todo: get this working (currently errors with: Index 0 out of bounds for length 0)
    ...
    if (global.sharedObj.resultsTextEditor == null) {
      const resultsTextEditor = atom.workspace.buildTextEditor();
      global.sharedObj.resultsTextEditor = resultsTextEditor;
      atom.workspace.getActivePane().splitRight({ items: [ resultsTextEditor ], copyActiveItem: false }).onWillDestroy(() => global.sharedObj.resultsTextEditor = null);
    }
...
//this should work, but the cql-cli needs to look for data below the library name
        ...
        this.executeCQL(editor, command, jarPath, libraryDirectory, libraryName, modelPathSpecifier, terminologyPath, contextSpecifier, context, measurementPeriod);
      });
  }
  ...
  async executeCQL(editor, command, jarPath, libraryDirectory, libraryName, modelPathSpecifier, terminologyPath, contextSpecifier, testName, measurementPeriod){
    //turn debug on or off here
    const debug = false;
    const args = this.getExecArgs(debug, jarPath, libraryDirectory, libraryName, modelPathSpecifier, terminologyPath, contextSpecifier, measurementPeriod);
...
//this results display should be in the caller, but I keep getting a null from the promis that's returned
        global.sharedObj.resultsTextEditor.insertText(`Test (${testName}) ERROR: see log for details\r\n${new Date().toString()}\r\n\r\n`);
        return;
      }
      console.error(`stderr: ${stderr}`);
//this results display should be in the caller, but I keep getting a null from the promis that's returned
      global.sharedObj.resultsTextEditor.insertText(`Test (${testName}) RESULTS:\r\n${stdout.toString()}${new Date().toString()}\r\n\r\n`);
    });
  }

  getExecArgs(debug, jarPath, libraryDirectory, libraryName, modelPathSpecifier, terminologyPath, contextSpecifier, measurementPeriod){
    return [
      debug ? '-Xdebug' : '',
