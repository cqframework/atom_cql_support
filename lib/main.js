...
//this should work, but the cql-cli needs to look for data below the library name
        ...
        this.executeCQL(editor, command, jarPath, libraryDirectory, libraryName, modelPathSpecifier, terminologyPath, contextSpecifier, context);
      });
  }
  ...
  async executeCQL(editor, command, jarPath, libraryDirectory, libraryName, modelPathSpecifier, terminologyPath, contextSpecifier, testName){
    //turn debug on or off here
    const debug = false;
    const args = this.getExecArgs(debug, jarPath, libraryDirectory, libraryName, modelPathSpecifier, terminologyPath, contextSpecifier, '');
    console.log(command + ' ' + args.join(' '));
    const connection = await this.getConnectionForEditor(editor.getModel());

//On Windows the args could error due to spaces in the path
