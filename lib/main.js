...
//todo: get this working (currently errors with: Index 0 out of bounds for length 0)
    ...
    atom.workspace.open(`${path.join(resultPath, libraryPathName + '.txt')}`,  {
      ...
      result.insertText(`${modelMessage}\r\n`);
      result.insertText(`${terminologyMessage}\r\n`);
      ...
      if (modelRootPath && modelRootPath != '') {
        ...
        if (dirs && dirs.length > 0) {
          dirs.forEach(dirent => {
            ...
            this.executeCQL(editor, command, jarPath, libraryDirectory, libraryName, fhirVersion, modelPathSpecifier, terminologyPath, contextSpecifier, context, measurementPeriod, verbose);
          });
        }
        else {
          this.executeCQL(editor, command, jarPath, libraryDirectory, libraryName, fhirVersion, null, terminologyPath, null, null, measurementPeriod, verbose);
        }
      }
      else {
        this.executeCQL(editor, command, jarPath, libraryDirectory, libraryName, fhirVersion, null, terminologyPath, null, null, measurementPeriod, verbose);
  ...
  async executeCQL(editor, command, jarPath, libraryDirectory, libraryName, fhirVersion, modelPathSpecifier, terminologyPath, contextSpecifier, testName, measurementPeriod, verbose){
