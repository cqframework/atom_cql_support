...
//this should work, but the cql-cli needs to look for data below the library name
  ...
  async executeCQL(editor, command, jarPath, libraryDirectory, libraryName, modelPathSpecifier, terminologyPath, contextSpecifier, testName, measurementPeriod){
    ...
    const debug = false;
    const args = this.getExecArgs(jarPath, libraryDirectory, libraryName, modelPathSpecifier, terminologyPath, contextSpecifier, measurementPeriod, debug);
...
//this results display should be in the caller, but I keep getting a null from the promise that's returned
  ...
  getExecArgs(jarPath, libraryDirectory, libraryName, modelPathSpecifier, terminologyPath, contextSpecifier, measurementPeriod, debug){
    return [
      debug ? '-Xdebug' : '',
