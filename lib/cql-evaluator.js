...
async function executeCQLFile(editor, verbose) {
    ...
    let args = getJavaJarArgs(jarPath);
    args = getCqlCommandArgs(args, fhirVersion);
    ...
    if (modelRootPath && modelRootPath != '') {
        ...
        if (dirs && dirs.length > 0) {
            dirs.forEach(async (dirent) => {
                ...
                args = getExecArgs(args, libraryDirectory, libraryName, modelType, modelPath, terminologyPath, context, measurementPeriod);
            });
        }
        else {
            args = getExecArgs(args, libraryDirectory, libraryName, modelType, null, terminologyPath, null, measurementPeriod);
        }
    }
    else {
        args = getExecArgs(args, libraryDirectory, libraryName, modelType, null, terminologyPath, null, measurementPeriod);
    ...
    await executeCQL(textEditor, editor, command, args);
}
...
async function executeCQL(textEditor, editor, command, args) {
    console.log(command + ' ' + args.join(' '));

    const startExecution = new Date();
    const cql = cp.spawn(command, args, { stdio: ['ignore', 'pipe', 'pipe'] });
    await echoToTextEditor(textEditor, cql.stdout);

    const endExecution = new Date();

    await echoToConsole(cql.stderr);
    ...
    cp.exec(`"${command}" ${args.join(' ')}`, async (error, stdout, stderr) => {
        ...
        if (error) {
            ...
            await textEditor.insertText(`${new Date().toString()}\r\n${error}\r\n`);
            return;
        }
...
async function echoToTextEditor(textEditor, readable) {
    for await (const line of chunksToLines(readable)) {
        await textEditor.insertText(line);
    }
}

async function echoToConsole(readable) {
    for await (const line of chunksToLines(readable)) {
        console.log(line);
    }
}

function getJavaJarArgs(jarPath) {
    var args = [];
    // if (debug) {
    //     args.push('-Xdebug');
...
function getCqlCommandArgs(args, fhirVersion) {
    args.push("cql")
...
function getExecArgs(args, libraryDirectory, libraryName, modelType, modelPath, terminologyPath, contextValue, measurementPeriod) {
